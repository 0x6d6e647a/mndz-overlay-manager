...
After pycargoebuild inplace update on full path (or on content repair), the program SHALL rewrite the ebuild `SRC_URI` to the steady-state two-line form for the package’s Cargo provenance and SHALL NOT rely on `${CARGO_CRATE_URIS}` as the dependency distfile source for steady-state tarball-shaped ebuilds. Provenance `CargoGitTag`: the primary source line is the upstream GitHub source archive for the tag. Provenance `CargoCratesIo`: the primary source line is the canonical crates.io download distfile for the policy crate name and PV, `https://crates.io/api/v1/crates/<crate>/<pv>/download -> <p>.crate`. Both provenances: the secondary line is the mndz-overlay-assets crates tarball URL for `{pn}-${PV}-crates.tar.xz`.
