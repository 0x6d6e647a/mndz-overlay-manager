...
**Reason**: Signing no longer caches the passphrase in the desktop agent. Clearing a desktop keygrip is the wrong teardown, and it does not cover an agent this process started.
**Migration**: Terminate the session agent on exit and on parent death. Leave the desktop agent unchanged.
