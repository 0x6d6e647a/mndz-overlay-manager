...
4. That GitHub always includes read-only access to public repositories on a fine-grained PAT, so `github-token` SHALL still accept a PAT whose only Contents: write grant is the assets origin even when the operator owns other public repositories.
...
#### Scenario: README names public-repo read vs Contents write

- **WHEN** an operator reads how to create the GitHub PAT for `github-token`
- **THEN** README states that GitHub’s always-on public-repository read does not fail the setter, and that Contents: write must be limited to the assets origin
