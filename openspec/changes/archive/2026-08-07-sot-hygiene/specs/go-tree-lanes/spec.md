...
**Reason**: Exact-set prune after successful multi-PV `DepsAndAssets` apply is owned by `runtime-lanes` (“Exact-set package directory for DepsAndAssets”) and `update-apply` multi-lane apply; restating it only for Go risked divergent wording.

**Migration**: Use `runtime-lanes` exact-set requirement and `update-apply` multi-lane apply for prune behavior across ecosystems including Go.
