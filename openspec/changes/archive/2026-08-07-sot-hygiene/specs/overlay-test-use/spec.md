...
- **WHEN** an ebuild is modified only for IUSE, RESTRICT, `src_test`, or other non-PV content
...
The mndz-overlay package `dev-util/ralph-tui` SHALL declare `IUSE` including `test` and SHALL set `RESTRICT` including `!test? ( test )`. `src_test` SHALL invoke the package test command (`bun test`) when the test phase runs. Content-only fixes that introduce or adjust these tokens SHALL ship as a revision bump of the current PV.

#### Scenario: RESTRICT present after revbump

- **WHEN** the live ralph-tui ebuild is inspected
- **THEN** it is a `-rN` ebuild for the prior PV (or higher N if already revised) when the gate was introduced as content-only
...
- **WHEN** the live hk and usage ebuilds are inspected
- **THEN** each satisfies the same IUSE, RESTRICT, cargo test, and revision rules as mise
