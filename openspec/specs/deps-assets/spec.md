...
Parameterized asset SRC_URI SHALL mean package-owned assets-repository release download URLs only. The assets repository is `{owner}/{repo}` parsed from `assets-path` `origin` as specified by `assets-publish`. The download marker is `{repo}/releases/download/`. A release tag after that marker is package-owned when it contains `${PV}` or starts with `{pn}-`. Those tags, and filenames under them that start with `{pn}-`, SHALL use `${PV}` (not a frozen overlay package version). Assets-host tags that are not package-owned are a different version axis: they SHALL NOT be rewritten to `{pn}-${PV}`, and their lack of `${PV}` SHALL NOT by itself make the PV need work. Non-assets `SRC_URI` entries (no assets-repo release download marker for that `{repo}`) remain out of this check. Write and check SHALL use the same ownership test. The program SHALL NOT treat a different GitHub repository’s release URLs as assets-host URLs solely because they contain `releases/download/`.
...
- **WHEN** a present planned PV's canonical ebuild has a parameterized `{pn}-${PV}` crates (or vendor/deps) assets URL and a second assets-repo URL whose release tag is `rusty-v8-${RUSTY_V8_VER}` (no `${PV}`)
- **THEN** that second URL does not by itself make the PV need work

#### Scenario: Frozen package-owned assets URL still needs work

- **WHEN** a present planned PV's canonical ebuild has `…/{repo}/releases/download/{pn}-1.2.3/{pn}-1.2.3-crates.tar.xz` (or vendor/deps equivalent) with no `${PV}` for the origin `{repo}`
