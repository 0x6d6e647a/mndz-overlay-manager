...
Haskell Program Coverage (HPC) instrumentation and reporting for the tasty suite: Overall / Unit / Integration breakdowns, product-module scope with scaffolding excludes, human and machine report artifacts, and coverage gate success without numeric floors.
...
1. **Pure and CLI** — option resolution, work-command parse edges (including `github-token` and `--force`), preflight pure helpers, version-tag and SSH-identity pure helpers, config load error messaging and path selection, overlay validation failures, GitHub token resolution edges (envelope vs plaintext vs env, env warnings, decrypt when live `api.github.com` work is required, skip-decrypt when this run will not call GitHub), `github.com` origin owner/repo parse, `mndz1.` envelope detect/round-trip with a test wrapping password, GitHub HTTP error formatting (status, JSON message, rate-limit headers), rate-limit/401 classification, and technique/ecosystem/package-key pure helpers; logging bootstrap construction for controlled verbosity and color.
...
3. **Check and plan** — product Check and Deps.Plan pipelines for DepsAndAssets ecosystems Go, Npm, Bun, and Cargo with injectable fetchers/plan ops; at least one multi-package or multi-phase workflow under Integration; GitHub health skip on valid check-cache hits; command abort on fake rate-limit/401 including completed `outdated` stdout then exit `1`.
...
- **WHEN** coverage-oriented suite tests for builders, plan/check, materialize, HTTP clients, token probe, GitHub health, and agents run in the quality gate
- **THEN** they complete without calling public registries, live GitHub APIs, live Statuspage, or interactive pinentry for success
