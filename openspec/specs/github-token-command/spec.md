...
2. Paginated `GET /user/repos?affiliation=owner` returns exactly that one `owner/repo` (no extra owned repositories).
...
- **WHEN** empty `POST /releases` returns HTTP 403
- **THEN** the program hard-fails without writing `github-token`

#### Scenario: Extra owned repos refused

- **WHEN** `GET /user/repos?affiliation=owner` lists the assets repo and any other repository
- **THEN** the program hard-fails without writing `github-token`
